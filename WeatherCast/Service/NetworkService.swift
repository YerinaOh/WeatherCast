...
class NetworkService: NSObject {
    ...
    func updateSearchResults(searchText: String?, completion:@escaping ([RegionModel]) -> Void){
    ...
    func loadCurrentRegion(latitude: Double, longitude: Double, completion:@escaping (RegionModel?) -> Void) {
    ...
    func loadWeatherWithIDs(ids: String, successHandler: @escaping ([GroupWeatherListModel]?) -> Void, errorHandler: @escaping (Error?) -> Void) {
        ...
        let url: URL = URL.init(string: strUrl)!
            ...
            self.loadWeatherData(url: url, successHandler: { (data) in
                if let data = data {
                    do {
